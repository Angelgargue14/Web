...
TIPO_DOCUMENTO ENUM('NIF', 'CIF') NOT NULL COMMENT 'Tipo de documento: NIF para persona, CIF para empresa',
IDENTIDAD VARCHAR(9) NOT NULL UNIQUE COMMENT 'NIF o CIF del cliente, único',
FECHA_NACIMIENTO DATE COMMENT 'Fecha de nacimiento del cliente (opcional, útil para personas físicas)',
CORREO VARCHAR(120) COMMENT 'Correo electrónico del cliente',
TELEFONO VARCHAR(15) COMMENT 'Teléfono del cliente',
DIRECCION VARCHAR(255) COMMENT 'Dirección fiscal del cliente',
...
PAIS VARCHAR(100) COMMENT 'País',
RAZON_SOCIAL VARCHAR(255) COMMENT 'Razón social de la empresa (solo si es tipo CIF)',
TIPO_CLIENTE ENUM('PARTICULAR','EMPRESA','DISTRIBUIDOR') DEFAULT 'PARTICULAR' COMMENT 'Clasificación del cliente',
MOROSO BOOLEAN DEFAULT FALSE COMMENT 'FALSE = no moroso, TRUE = moroso',
ALTA DATE NOT NULL COMMENT 'Fecha de alta del cliente',
MODIFICACION DATETIME NOT NULL COMMENT 'Fecha de última modificación',
OBSERVACIONES TEXT COMMENT 'Observaciones adicionales sobre el cliente',
CONSTRAINT CLIENTES_PK PRIMARY KEY (ID_CLIENTE)
) COMMENT='TABLA DE CLIENTES CON DATOS COMPLETOS PARA FACTURACIÓN';

CREATE TABLE EMPLEADOS(
ID_EMPLEADO VARCHAR(33) COMMENT 'CODIGO INTERNO DEL EMPLEADO, MD5 DEL CAMPO IDENTIDAD',
NUM_EMPLEADO INT AUTO_INCREMENT UNIQUE COMMENT 'NUMERO DE EMPLEADO',       
...
ID_PEDIDO VARCHAR(16) COMMENT 'CODIGO DE PEDIDO',
ID_CLIENTE VARCHAR(33) COMMENT 'MD5 DEL CAMPO IDENTIDAD.CLIENTES, IDENTIFICADOR DEL CLIENTE',
ID_EMPLEADO VARCHAR(33) NOT NULL     COMMENT 'CODIGO INTERNO DEL EMPLEADO, MD5 DEL CAMPO IDENTIDAD',
MESA VARCHAR(4) COMMENT 'NUMERO DE MESA',
...
FECHA_PAGO DATETIME COMMENT 'FECHA DE PAGO DEL PEDIDO',
...
CONSTRAINT MESA_FK FOREIGN KEY (MESA) REFERENCES MESAS (MESA)
);
...
DIMENSIONES VARCHAR(100) NOT NULL C             O   MMENT 'Dimensiones tipo AAxLLxFF (alto, largo y fondo)',
...
-- Asigna el ID de la categoría como el MD5 de su nombre
...
-- Asigna el ID de la subcategoría como el MD5 de su nombre
...
-- Asigna el ID del artículo como el MD5 de su nombre
...
-- Asigna el ID del empleado como el MD5 de su identidad (DNI/NIF/NIE)
...
-- Asigna la fecha y hora actual al campo FECHA_COMPRA al insertar un pedido
...
-- Asigna la fecha de creación y modificación actual al insertar una reserva
...
-- Actualiza la fecha de modificación al editar una reserva
...
-- Asigna la fecha de inserción y descuento por defecto (0) al crear un carrito
